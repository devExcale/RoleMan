...
public class RoleAddHandler implements GuildMessageReactionHandler {
	...
	public void onReactionAdd(GuildMessageReactionAddEvent event) {
		...
		reaction.getChannel().retrieveMessageById(messageId).queue(message -> {
			...
			if(message.getAuthor().getId().equals(RoleMan.selfUser().getId()) && embeds.size() == 1) {
				...
				member.getUser().openPrivateChannel().queue(dm -> {
					...
					// Role not found
					if(role == null) {
						dm.sendMessageFormat("The role you're trying to get in guild `%s` doesn't exist anymore.",
								guildName).queue();
						return;
					}
					...
					guild.addRoleToMember(member, role).queue(
							...
							// Role give success
							unused -> dm.sendMessageFormat("I've given you the role `%s` on guild `%s`!",
									roleName,
									guildName).queue(),

							// Role give fail
							throwable -> dm.sendMessageFormat("I couldn't give you the role `%s` on guild `%s`.",
									roleName,
									guildName).queue());
